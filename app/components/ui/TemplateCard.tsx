  ...
  return (
    <div
      className={`
        ...
        ${featured ? "ring-2 ring-orange-500/30 bg-gradient-to-br from-yellow-50/50 to-orange-50/50" : ""}
        ${className}
      `}
    ...
    >
      ...
      {featured && (
        <div className="absolute top-1 right-1 z-10">
          <div className="flex items-center px-3 py-1.5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white text-xs font-semibold rounded-full shadow-lg">
      ...
      <div className={`flex items-start space-x-4 mb-4 flex-1  ${featured ? "mt-2" : ""}`}>
        ...
        <div className=" min-w-0">
          ...
          <p className="text-sm text-gray-600 line-clamp-3 leading-relaxed">{template.description}</p>
        </div>
      </div>
      ...
      {template.tags && template.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-auto">
          ...
          {template.tags.length > 3 && (
            <span className="px-2 py-1 text-xs text-gray-400 bg-gray-50 rounded-md">+{template.tags.length - 3}</span>
          )}
        </div>
      )}
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/5 to-orange-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl pointer-events-none" />
    </div>
  );
};
